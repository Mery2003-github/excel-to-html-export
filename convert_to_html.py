...
def get_image_data(zipf, media_path):
    try:
        ...
        return f"data:image/webp;base64,{base64.b64encode(webp_data).decode()}"
        ...
        with zipf.open(media_path) as f:
            raw_data = f.read()
        return f"data:{mime_fallback};base64,{base64.b64encode(raw_data).decode()}"
...
def get_column_widths(wb, sheet_name=None):
    ws = wb[sheet_name] if sheet_name else wb.active
    return {
        col: (ws.column_dimensions.get(get_column_letter(col)).width or 8.43)
...
def get_row_heights(wb, sheet_name=None):
    ws = wb[sheet_name] if sheet_name else wb.active
    return {
        row: (ws.row_dimensions.get(row).height or 15)
        ...
        for anchor in root.findall('xdr:oneCellAnchor', ns) + root.findall('xdr:twoCellAnchor', ns):
            try:
                ...
                images.append({
                    ...
                    'width': width,
                    'height': height,
                    'data_uri': get_image_data(zipf, media_path)
                })
            except Exception as e:
...
"""
    for row_idx, row in enumerate(sheet_data, 1):
        for col_idx, cell in enumerate(row, 1):
            ...
            html += f"""
<div class="cell" style="left:{left}px; top:{top}px; width:{width}px; height:{height}px; {style}">
    {cell['value']}
</div>
"""
    for img in images:
        html += f"""
<div class="image-container" style="left:{img['left']}px; top:{img['top']}px; width:{img['width']}px; height:{img['height']}px;">
